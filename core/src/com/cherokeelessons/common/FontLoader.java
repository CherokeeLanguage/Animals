...
public class FontLoader {
	...
	public FontLoader() {
		...
		cache_fonts = new HashMap<>();
		cache_fonts_fixed = new HashMap<>();
	}

	public BitmapFont get(final int size) {
		final String fileName = "fonts/script-heavy-" + size + ".fnt";
		Gdx.app.log(this.getClass().getName(), "get: " + fileName);
		if (!fonts.isLoaded(fileName)) {
			fonts.load(fileName, BitmapFont.class, param);
			fonts.finishLoading();
			final BitmapFont bitmapFont = fonts.get(fileName, BitmapFont.class);
			cache_fonts.put(fileName, bitmapFont);
		}
		return cache_fonts.get(fileName);
	}

	public BitmapFont getFixed(final int size) {
		final String fileName = "fonts/script-heavy-" + size + ".fnt";
		...
		if (!fonts_fixed.isLoaded(fileName)) {
			...
			final BitmapFont bitmapFont = fonts_fixed.get(fileName, BitmapFont.class);
