...
if __name__ == "__main__":
    ...
    dest_script = os.path.join(android_assets, "text", "challenges.txt")

    pngs: dict[str, list[str]] = get_png_lookup(os.path.join(base_dir, "pictures/challenges"))
    ...
    for png in get_pngs(base_dir):
        dest_image: str = os.path.join(dest, os.path.basename(png))
        ...
        png.save(dest_image)
        #shutil.copy2(png, dest_image)

    with open(dest_script, "w") as f:
        f.write("extends Node\n")
        f.write("\n")
        f.write("class_name AnimalPngs\n")
        f.write("\n")
        ...
        for challenge in pngs.keys():
            ...
            if challenge in syl_map:
                f.write(syl_map[challenge])
            elif challenge.replace(" ", "_") in syl_map:
                f.write(syl_map[challenge.replace(" ", "_")])
            else:
                f.write(challenge)
            f.write("\": [")
